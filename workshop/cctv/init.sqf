...
@date 20130707
*****************************************************************************/
private ["_laptop", "_camera_objs", "_player_camera", "_get_pos", "_get_dir", "_cam", "_first_cam", "_x", "_y", "_z", "_cam_next", "_cam_rotateplus",
		"_cam_rotateminus", "_cam_quit", "_loop", "_cam_i", "_cams_n", "_inc", "_next_cam", "_dir", "_nv", "_hasGPS"];

CCTV_pan_angle = 45; 	// Angle the camera moves left to right
CCTV_tilt_angle = 15; 	// Angle the camera is tilted down
CCTV_time_int = 5; 		// Time the camera holds
CCTV_radius = 250; 		// Radius which notebooks detect cameras
...
while {_loop} do {
	if (CCTV_NEXT) then {
		...
		_movehandle = [_cam, _positions] execVM "scripts\workshop\cctv\move.sqf";
